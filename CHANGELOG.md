...
- **[2026-03-26] Playwright E2E Suite**: Established a full end-to-end test suite with 6 spec files covering splash/auth, patient directory, layout, medical chart, live dictation, and AI integration.
- **[2026-03-26] Clinical Rules Engine**: Implemented `RulesEngineService` enforcing 9 runtime guardrails with context-aware modes (dyslexia, pediatric, casual) and an `activeContexts` signal.

### Changed

- **[2026-03-26] Test Infrastructure Consolidation**: Unified all Playwright config, spec files, reports, results, and helper scripts under `tests/`. Updated `playwright.config.ts` to anchor `outputDir`, `playwright-report/`, and `report.json` under `tests/`. Removed root-level `playwright-report/` and `test-results/` directories.
